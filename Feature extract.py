...
sub_arrays0_qian = label_1[:split_point0]
sub_arrays0_hou = label_1[-split_point0:]
# 新增
...
data_for_train = np.array(test_dataset)#！！！3


train_str=data_for_train[0:half_of_test, 0:-1]
train_data=[]
...
train_labels_str=data_for_train[0:half_of_test, -1]
...
class GCN(torch.nn.Module):
    def __init__(self, num_node_features, num_classes):
        ...
        self.conv1 = GCNConv(num_node_features, 64)  # 第一层GCN，输出维度16
        self.bn2 = BatchNorm1d(64)
        self.conv2 = GCNConv(64, num_classes)
        self.conv3 = GCNConv(2, 64)
...
def train():
    for epoch in range(150):
        ...
        if acc >= 0.9:
            ...
            # 保存模型
            torch.save(model.state_dict(), f'./models/tyy_{filename}.pt')
            best_accuracy = acc
            best_epoch = epoch + 1
            break
...
def feature_extract():
    ...
    # 加载已保存的模型参数
    model.load_state_dict(torch.load(f'./models/tyy_{filename}.pt'))
    ...
    # 将DataFrame保存为CSV文件
    df.to_csv(f'./output/feature64/{filename}.csv', index=False)  #将索引列（行号）排除在CSV文件之外
